...
export const getUserProfile: express.RequestHandler = async (req, res) => {
    try {
        const {id} = req.params;
...
export const follow_user: express.RequestHandler = async (req, res) => {
    ...
    try {
        ...
        if(!other_user) return res.status(404).json({
        ...
        if(is_follow_exist) return res.status(400).json({
...
export const unfollow_user: express.RequestHandler = async (req, res) => {
    const {user_id} = req.params;
    try {
        if(!user_id) return res.status(400).json({
            error: 'user id is required'
        ...
        if(!follow_exist) return res.status(400).json({
...
export const delete_account: express.RequestHandler = async (req, res) => {
    const {user_id} = req.params
    try {
        if(!user_id) return res.status(400).json({
            error: "user id is required"
...
export const update_proile: express.RequestHandler = async (req, res) => {
    const {username, last_name, first_name, email } = req.body
    const {user_id} = req.params
    try {
        if(!username && !last_name && !first_name && !email) return res.status(400).json({
            error: "bad request"
        })
